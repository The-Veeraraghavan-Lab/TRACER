...
class VxmDense_3D_LSTM_Step_Reg_old(LoadableModel):
    """
    VoxelMorph network for (unsupervised) nonlinear registration between two images.
    """
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_w_dose(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in,plandose):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        #print ('plandose size',plandose.size())
        #print ('planct_x size',planct_x.size())
        source_dose=self.transformer_range_flow(plandose, pos_flow,range_flow)
        ...
        return source, pos_flow,source_deformed_list,source_m_deformed_list,positive_deform_list,h,c,source_m,flow_acc_seg.detach(),gtv_in,source_dose
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_gtv_not_deform(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_gtv_not_deform_all_metric(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtin_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        source_m = self.transformer_nearest_range_flow(planct_y, pos_flow,range_flow)
        gtv_in=self.transformer_nearest_range_flow(gtv_in, pos_flow,range_flow)
        cbct_gtin_in=self.transformer_nearest_range_flow(cbct_gtin_in, -pos_flow,range_flow*2)
        ...
        return source, pos_flow,source_deformed_list,source_m_deformed_list,positive_deform_list,h,c,source_m,flow_acc_seg.detach(),gtv_in,cbct_gtin_in
    ...
    def forward_with_acc_flow_correction_range_flow_gtv_in(self, source, source_m,target,states,flow_ini,plan_ct_img,planct_val_msk,planct_gtv,range_flow):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
    ...
    def forward_with_acc_flow_correction_range_flow_gtv_in_def_not_accmu_(self, source, source_m,target,states,flow_ini,plan_ct_img,planct_val_msk,planct_gtv,range_flow):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        pos_flow_out=pos_flow+flow_ini
        if self.resize:
            pos_flow_cur = self.resize(pos_flow_cur)
            pos_flow = self.resize(pos_flow_out)
        ...
        if self.integrate:
            #print ('pos flow size is ',pos_flow.size())
            pos_flow = self.integrate_range_flow(pos_flow,range_flow)
            pos_flow_cur = self.integrate_range_flow(pos_flow_cur,range_flow)
            ...
            if self.fullsize:
                pos_flow = self.fullsize(pos_flow)
                pos_flow_cur = self.fullsize(pos_flow_cur)
                #neg_flow = self.fullsize(neg_flow) if self.bidir else None
        
        flow_acc=pos_flow_out#+flow_ini
        source = self.transformer_range_flow(source, pos_flow_cur,range_flow)
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in(self, source, target,source_m,flow_num,h,c,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
...
class VxmDense_3D_no_clstm(LoadableModel):
    """
    VoxelMorph network for (unsupervised) nonlinear registration between two images.
    """
    ...
    def __init__(self,
        ...
        # configure core unet model
        self.unet_model = Unet_3D_no_LSTM(
        #self.unet_model = Unet_3D_LSTM(    
            inshape,
            nb_features=nb_unet_features,
            nb_levels=nb_unet_levels,
        ...
        self.grid_template=torch.zeros(1, 1,128,128,96)
        grid_w=12
        for i in range(0,26):
                    
            self.grid_template[:,:,:,i*5,:]=1
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_grid(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in,grid_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        #gtv_zero=torch.zeros(gtv_in.size()).cuda()

        #x=torch.cat([x, gtv_zero], dim=1)
        #x=torch.cat([x, gtv_zero], dim=1)
        ...
        flow_acc_seg=flow_acc_seg+pos_flow
        # warp image with flow field
        source = self.transformer_range_flow(planct_x, flow_acc_seg,range_flow)
        source_m = self.transformer_nearest_range_flow(planct_y, flow_acc_seg,range_flow)

        gtv_in=self.transformer_nearest_range_flow(gtv_in, flow_acc_seg,range_flow)

        grid_template=self.transformer_range_flow(self.grid_template, flow_acc_seg,range_flow)
        ...
        return source, pos_flow,source_deformed_list,source_m_deformed_list,positive_deform_list,h,c,source_m,flow_acc_seg.detach(),gtv_in,grid_template



    def forward_with_out_acc_flow_correction_range_flow_gtv_in_preint_flow_out(self, source, source_m,target,state_h_in,state_c_in,flow_ini,plan_ct_img,planct_val_msk,source_gtv_def,range_flow,cbt_gtv,neg_flow_nin,target_def):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        x=  torch.cat([x, source_gtv_def], dim=1)
        x=  torch.cat([x, cbt_gtv], dim=1)
        
        #print ('input x size is ',x.size())
        #print (states.size())
        x,state_h,state_c = self.unet_model(x,None)
        ...
        source = self.transformer_range_flow(source, pos_flow,range_flow)
        source_m = self.transformer_nearest_range_flow(source_m, pos_flow,range_flow)

        source_gtv_def=self.transformer_nearest_range_flow(source_gtv_def, pos_flow,range_flow) 

        target_def = self.transformer_range_flow(target_def, neg_flow,range_flow)

        #states=[h.detach(),c.detach()]

        return source, source_m,source_gtv_def, preint_pos_flow,state_h,state_c,flow_acc.detach(),preint_neg_flow,neg_flow_acc.detach(),target_def
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_Not_Used_20221205(self, source, target,source_m,flow_num,h,c,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
...
class VxmDense_3D_LSTM_Step_Reg_All_Enc_Lstm_target_gtv_in(LoadableModel):
    ...
    def __init__(self,
        ...
        bidir=False,
        int_downsize=2,
        dropout_flag=0, #Here   we used default dropout flag to 0 to not drop out, dropout used only for perry re-radiation patients, REMBER to change flag to 0 back for normally inter-p registration
        use_probs=False):
        """ 
        Parameters:
            inshape: Input shape. e.g. (192, 192, 192)
        ...
        if self.dropout_flag==1:
            self.unet_model = Unet_All_Encoder_3D_LSTM_target_gtv_in_dropout(
            ...
            #self.unet_model = Unet_All_Encoder_3D_LSTM_target_gtv_in_dropout(
            self.unet_model = Unet_All_Encoder_3D_LSTM_target_gtv_in(
        ...
        self.integrate_range_flow = layers.VecInt_range_flow(down_shape, int_steps) if int_steps > 0 else None
        self.integrate_range_flow_steps = layers.VecInt_range_flow_Steps(down_shape, int_steps) if int_steps > 0 else None
    ...
    def forward_with_acc_flow_correction_range_flow_gtv_in_preint_flow_out(self, source, source_m,target,state_h_in,state_c_in,flow_ini,plan_ct_img,planct_val_msk,planct_gtv,range_flow,cbt_gtv,neg_flow_nin):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
    ...
    def forward_with_acc_flow_after_inter_range_flow_gtv_in_preint_flow_out(self, source, source_m,target,state_h_in,state_c_in,flow_ini,plan_ct_img,planct_val_msk,planct_gtv,range_flow,cbt_gtv,neg_flow_nin):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
    ...
    def forward_with_out_acc_flow_correction_range_flow_gtv_in_preint_flow_out_old(self, source, source_m,target,state_h_in,state_c_in,flow_ini,plan_ct_img,planct_val_msk,planct_gtv,range_flow,cbt_gtv,neg_flow_nin):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in):
        ...
        #x=torch.cat([x, gtv_in], dim=1)
        #x=torch.cat([x, cbct_gtv_in], dim=1)
        zeros_tensor=torch.zeros(source.size()).cuda()
        ...
        # resize flow for integration
        pos_flow = flow_field # current one 
        # accumulate with previous one
        flow_acc_seg=pos_flow+flow_acc_seg
        if self.resize:
            pos_flow = self.resize(flow_acc_seg)
    ...
    def forward_seg_training_flow_acc_after_inter_range_flow_gtv_in(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
    ...
    def forward_seg_training_flow_acc_after_inter_range_flow_gtv_in_w_dose(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in,plandose):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        #print ('plandose size',plandose.size())
        #print ('planct_x size',planct_x.size())
        source_dose=self.transformer_range_flow(plandose, flow_acc_seg,range_flow)
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_grid_all_metric(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in,grid_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        source_m = self.transformer_nearest_range_flow(planct_y, flow_acc_seg,range_flow)
        gtv_in=self.transformer_nearest_range_flow(gtv_in, flow_acc_seg,range_flow)
        cbct_gtv_in=self.transformer_nearest_range_flow(cbct_gtv_in, flow_acc_seg,range_flow)
        grid_template=self.transformer_range_flow(self.grid_template, flow_acc_seg,range_flow)
        ...
        return source, pos_flow,source_deformed_list,source_m_deformed_list,positive_deform_list,h,c,source_m,flow_acc_seg.detach(),gtv_in,grid_template,cbct_gtv_in
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_grid_step_intern(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in,grid_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_grid_step_intern_itern_Steps(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in,grid_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        if self.integrate:
            #print ('pos flow size is ',pos_flow.size())
            pos_flow_steps,vec_final = self.integrate_range_flow_steps(pos_flow,range_flow)
            
            #print ('vec_final size ',vec_final.size())
            #print('pos_flow_steps[i] before',pos_flow_steps[0].size())
            # resize to final resolution
            flow_len=len(pos_flow_steps)
            if self.fullsize:
                
                vec_final= self.fullsize(vec_final)# + flow_acc_seg
                #print ('vec_final full size ',vec_final.size())
                for i in range (flow_len):
                    #print('pos_flow_steps[i] before',pos_flow_steps[i].size())
                    pos_flow_steps[i] = self.fullsize(pos_flow_steps[i]) #+ flow_acc_seg
                    
                    #print('flow_acc_seg size',flow_acc_seg.size())
                    #print('pos_flow_steps[i]',pos_flow_steps[i].size())

                    pos_flow_steps[i]=pos_flow_steps[i]+flow_acc_seg
                    
        flow_acc_seg_final=flow_acc_seg+vec_final        
        neg_flow_acc=[]
        for i in range(flow_len):
            neg_flow_acc_cur=flow_acc_seg_final-pos_flow_steps[i] + flow_acc_seg
            neg_flow_acc.append(neg_flow_acc_cur)        

        #flow_acc_seg=flow_acc_seg+pos_flow

        flow_acc_seg_ori_in=flow_acc_seg
        flow_acc_seg=flow_acc_seg+vec_final
        ...
        for i in range(flow_len):
            pos_flow_acc_step.append(pos_flow_steps[i]-flow_acc_seg_ori_in)
            neg_flow_acc_step.append(neg_flow_acc[i]-flow_acc_seg_ori_in)

        pos_flow_acc_step=pos_flow_steps
        neg_flow_acc_step=neg_flow_acc

        source_pos_step=[]
        grid_template_pos_step=[]

        source_neg_step=[]
        grid_template_neg_step=[]
        for i in range(flow_len):
            cur_pos_step=pos_flow_steps[i]
            cur_neg_step=neg_flow_acc[i]

            source_pos_step_cur = self.transformer_range_flow(planct_x, cur_pos_step,range_flow)
            
            #grid_template_pos_step_cur=self.transformer_range_flow(self.grid_template, cur_pos_step,range_flow)
            grid_template_pos_step_cur=self.transformer_range_flow(self.grid_template, pos_flow_acc_step[i],range_flow)

            source_neg_step_cur = self.transformer_range_flow(planct_x, cur_neg_step,range_flow)
            
            #grid_template_neg_step_cur=self.transformer_range_flow(self.grid_template, cur_neg_step,range_flow)
            grid_template_neg_step_cur=self.transformer_range_flow(self.grid_template, neg_flow_acc_step[i],range_flow)

            source_pos_step.append(source_pos_step_cur)
            grid_template_pos_step.append(grid_template_pos_step_cur)

            source_neg_step.append(source_neg_step_cur)
            grid_template_neg_step.append(grid_template_neg_step_cur)
        ...
        return source, pos_flow,h,c,source_m,flow_acc_seg.detach(),gtv_in,grid_template,source_pos_step,grid_template_pos_step,source_neg_step,grid_template_neg_step,pos_flow_acc_step,neg_flow_acc_step



    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_grid_step_intern_itern_Steps_single(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in,grid_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        neg_flow_acc=[]
        for i in range(flow_len):
            neg_flow_acc_cur=vec_final-pos_flow_steps[i] #+ flow_acc_seg
            neg_flow_acc.append(neg_flow_acc_cur)        
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_grid_no_accumu_flow(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in,grid_in):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_grid_TRE_Points(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_gtv_in,grid_in,move_img_tre_points):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        source_m = self.transformer_nearest_range_flow(planct_y, flow_acc_seg,range_flow)
        gtv_in=self.transformer_nearest_range_flow(gtv_in, flow_acc_seg,range_flow)
        move_img_tre_points=self.transformer_nearest_range_flow(move_img_tre_points, flow_acc_seg,range_flow)
        grid_template=self.transformer_range_flow(self.grid_template, flow_acc_seg,range_flow)
        ...
        return source, pos_flow,source_deformed_list,source_m_deformed_list,positive_deform_list,h,c,source_m,flow_acc_seg.detach(),gtv_in,grid_template,move_img_tre_points
...
class Unet_3D_LSTM(nn.Module):
    ...
    def __init__(self, inshape, nb_features=None, nb_levels=None, feat_mult=1):
        ...
        #inter_p_reg 128_128_96 
        self.conv_lstm=ConvLSTM3D(input_channel=32, num_filter=32, b_h_w=(1,8,8,6), kernel_size=3,)  #[16,16,16]  [x,y,z]         
...
class Unet_3D_no_LSTM(nn.Module):
    """
    A unet architecture. Layer features can be specified directly as a list of encoder and decoder
    features or as a single integer along with a number of unet levels. The default network features
    per layer (when no options are specified) are:
...
class Unet_All_Encoder_3D_LSTM(nn.Module):
    ...
    def __init__(self, inshape, nb_features=None, nb_levels=None, feat_mult=1):
        ...
        #CBCT Tumor    
        self.conv_lstm=ConvLSTM3D(input_channel=32, num_filter=32, b_h_w=(1,8,8,6), kernel_size=3,)  #[16,16,16]  [x,y,z]
        self.conv_lstm4=ConvLSTM3D(input_channel=32, num_filter=32, b_h_w=(1,16,16,12), kernel_size=3,)
        self.conv_lstm3=ConvLSTM3D(input_channel=32, num_filter=32, b_h_w=(1,32,32,24), kernel_size=3,)
        self.conv_lstm2=ConvLSTM3D(input_channel=16, num_filter=16, b_h_w=(1,64,64,48), kernel_size=3,)
        self.conv_lstm1=ConvLSTM3D(input_channel=3, num_filter=3, b_h_w=(1,128,128,96), kernel_size=3,)
...
class Unet_All_Encoder_3D_LSTM_target_gtv_in_dropout(nn.Module):
    """
    A unet architecture. Layer features can be specified directly as a list of encoder and decoder
    features or as a single integer along with a number of unet levels. The default network features
    per layer (when no options are specified) are:
    ...
    def __init__(self, inshape, nb_features=None, nb_levels=None, feat_mult=1):
        ...
        self.conv_lstm3=ConvLSTM3D(input_channel=32, num_filter=32, b_h_w=(1,32,32,24), kernel_size=3,)
        self.conv_lstm2=ConvLSTM3D(input_channel=16, num_filter=16, b_h_w=(1,64,64,48), kernel_size=3,)
        self.conv_lstm1=ConvLSTM3D(input_channel=4, num_filter=4, b_h_w=(1,128,128,96), kernel_size=3,)
...
#
            ...
            if enc_lay_ct==2:
                ...
                #print (x_enc[-1].size())
                #print (h2.size())
                #print ('max h2 is ',h2.max())
                #print ('min h2 is ',h2.min())
                x_enc[-1]=h2
            if enc_lay_ct==3:
                if state_h_in == None:
                    h3,c3=self.conv_lstm3(x_enc[-1],None)
...
class Unet_All_Encoder_3D_LSTM_target_gtv_in(nn.Module):
    """
    A unet architecture. Layer features can be specified directly as a list of encoder and decoder
    features or as a single integer along with a number of unet levels. The default network features
    per layer (when no options are specified) are:
