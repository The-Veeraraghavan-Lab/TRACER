...
class VxmDense_3D_LSTM_Step_Reg_All_Enc_Lstm(LoadableModel):
    ...
    def forward_with_acc_flow_correction_range_flow_gtv_target_gtv_in(self, source, source_m,target,state_h_in,state_c_in,flow_ini,plan_ct_img,planct_val_msk,planct_gtv,range_flow,cbct_gtv):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
    ...
    def forward_seg_training_flow_acc_correction_range_flow_gtv_in_target_gtv(self, source, target,source_m,flow_num,h_in,c_in,flow_acc_seg,planct_x,planct_y,range_flow,gtv_in,cbct_val_gtv):
        '''
        Parameters:
            source: Source image tensor.
            target: Target image tensor.
        ...
        x = torch.cat([source, target], dim=1)
        x=torch.cat([x, gtv_in], dim=1)
        x=torch.cat([x, cbct_val_gtv], dim=1)
